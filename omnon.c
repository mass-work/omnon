...
void get_tb_virtual_keys(void) {
    tb_left_cpi             = calculate_cpi(virtual_keys[0][1][0]);
    tb_right_cpi            = calculate_cpi(virtual_keys[0][3][0]);
}
...
void get_report_add(report_add *tb_add_report, float delta_x, float delta_y, int orientation) {
    // 水平
    if (orientation == 1 || orientation == 3) {
        accumulated_h += (float)delta_x;
        accumulated_h2 = accumulated_h / 500 * X_SC_SCALE_FACTOR;
        // uprintf("accumulated_h2: %d\n" , (int)accumulated_h2*1000);
        if (fabs(accumulated_h2) >= 1.0f) {
            tb_add_report->lr = (accumulated_h2 > 0) ? 1 : -1;
            accumulated_h = 0; // Reset the accumulator
        }
    }
    // 垂直
    if (orientation == 2 || orientation == 4) {
        accumulated_v += (float)delta_y;
        accumulated_v2 = accumulated_v / 500 * Y_SC_SCALE_FACTOR;
        if (fabs(accumulated_v2) >= 1.0f) {
            tb_add_report->ud = (accumulated_v2 > 0) ? 1 : -1;
            accumulated_v = 0; // Reset the accumulator
        }
    }
...
void cursor_report(report_mouse_t *mouse_report, float delta_x, float delta_y) {
...
void tap_report(report_add *tb_add_report, uint8_t press_key1, uint16_t press_key2, uint16_t press_key3, uint16_t press_key4) {
...
void process_trackball_motion_move(bool motion_detected, uint8_t tb_mode, report_add *tb_add_report, report_mouse_t *mouse_report, int16_t delta_x, int16_t delta_y, int tb_orient, uint8_t press_key1, uint16_t press_key2, uint16_t press_key3, uint16_t press_key4) {
    if (motion_detected) {
        switch (tb_mode) {
            case KC_2:
                // arrow_report(tb_add_report, delta_x, delta_y, tb_orient);
                break;
            case KC_0:
                cursor_report(mouse_report, -delta_x, -delta_y);
                break;
            case KC_1:
                // scroll_report(mouse_report, tb_add_report, delta_x, delta_y, tb_orient);
                tap_report(tb_add_report, press_key1, press_key2, press_key3, press_key4);
...
void process_trackball_motion_press(bool *pressed, uint8_t row, uint8_t wait_time, uint8_t tb_orient, uint8_t current_layer, report_add *tb_add_report) {
...
// トラックボールの開放関数
    ...
    report_mouse_t pointing_device_task_kb(report_mouse_t mouse_report) {
        ...
        if (dual_motion_detected) {
            tb_wait_time = get_wait_time(virtual_keys[current_layer][4][1]);
            if (virtual_keys[current_layer][5][0] == KC_0) {
                get_report_add(&tb_add_report, report0.delta_x, report0.delta_y, tb_left_orient);
                process_trackball_motion_press(tb_d_pressed, 4, tb_wait_time, tb_right_orient, current_layer, &tb_add_report);
                process_trackball_motion_move(dual_motion_detected, virtual_keys[current_layer][4][0], &tb_add_report, &mouse_report, report0.delta_x, report0.delta_y, tb_left_orient, virtual_keys[current_layer][4][2], virtual_keys[current_layer][5][2], virtual_keys[current_layer][4][3], virtual_keys[current_layer][5][3]);
            } else {
                get_report_add(&tb_add_report, -report1.delta_x, -report1.delta_y, tb_right_orient);
                process_trackball_motion_press(tb_d_pressed, 4, tb_wait_time, tb_right_orient, current_layer, &tb_add_report);
                process_trackball_motion_move(dual_motion_detected, virtual_keys[current_layer][4][0], &tb_add_report, &mouse_report, -report1.delta_x, -report1.delta_y, tb_right_orient, virtual_keys[current_layer][4][2], virtual_keys[current_layer][5][2], virtual_keys[current_layer][4][3], virtual_keys[current_layer][5][3]);
            }

        } else if (left_motion_detected) {
            tb_wait_time = get_wait_time(virtual_keys[current_layer][0][1]);
            get_report_add(&tb_add_report, report0.delta_x, report0.delta_y, tb_left_orient);
            process_trackball_motion_press(tb_l_pressed, 0, tb_wait_time, tb_left_orient, current_layer, &tb_add_report);
            process_trackball_motion_move(left_motion_detected, virtual_keys[current_layer][0][0], &tb_add_report, &mouse_report, report0.delta_x, report0.delta_y, tb_left_orient, virtual_keys[current_layer][0][2], virtual_keys[current_layer][1][2], virtual_keys[current_layer][0][3], virtual_keys[current_layer][1][3]);

        } else if (right_motion_detected) {
            tb_wait_time = get_wait_time(virtual_keys[current_layer][2][1]);
            get_report_add(&tb_add_report, -report1.delta_x, -report1.delta_y, tb_right_orient);
            process_trackball_motion_press(tb_r_pressed, 2, tb_wait_time, tb_right_orient, current_layer, &tb_add_report);
        ...
        rxJoyData = (analogReadPin(ANALOG_JOYSTICK_RX_AXIS_PIN) - rxOrigin) / 4 * joy_r_speed_fact;
        ryJoyData = (analogReadPin(ANALOG_JOYSTICK_RY_AXIS_PIN) - ryOrigin) / 4 * joy_r_speed_fact;
        ...
        switch (virtual_keys[current_layer][6][0]) {
            case KC_0:
                ...
                if (lxJoyData > joy_orient_threshold || lyJoyData > joy_orient_threshold || lxJoyData < -joy_orient_threshold || lyJoyData < -joy_orient_threshold) {
                    ...
                    cursor_report(&mouse_report, -lxJoyData, lyJoyData);
                }else {
        ...
        switch (virtual_keys[current_layer][8][0]) {
            case KC_0:
                ...
                if (rxJoyData > joy_orient_threshold || ryJoyData > joy_orient_threshold || rxJoyData < -joy_orient_threshold || ryJoyData < -joy_orient_threshold) {
                    ...
                    cursor_report(&mouse_report, -rxJoyData, ryJoyData);
                }else {
